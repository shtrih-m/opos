...
type
  TFFDVersion = (ffdUnknown, ffd10, ffd105, ffd11, ffd12);
  TFFDVersions = set of TFFDVersion;

const
  ffdAll = [ffdUnknown, ffd10, ffd105, ffd11, ffd12];
  ffd11AndLess = [ffdUnknown, ffd10, ffd105, ffd11];
...
type
  ...
  public
    ...
    property Versions: TFFDVersions read FVersions;
...
begin
  ...
  AddTag(1102, 'сумма НДС чека по ставке 20%', 'СУММА НДС ЧЕКА 20%', ttVLN, 6);
  ...
  AddTag(1106, 'сумма НДС чека по расч. ставке 20/120', 'СУММА НДС ЧЕКА ПО РАСЧ. СТАВКЕ 20/120', ttVLN, 6);
  ...
  AddTag(1139, 'сумма НДС по ставке 20%', 'СУММА НДС 20%', ttVLN, 8);
  AddTag(1140, 'сумма НДС по ставке 10%', 'СУММА НДС 10%', ttVLN, 8);
  AddTag(1141, 'сумма НДС по расч. ставке 20/120', 'СУММА НДС ПО РАСЧ. СТАВКЕ 20/120', ttVLN, 8);
  ...
  // Старые теги
  AddTag(1148, 'количество самостоятельных корректировок', 'КОЛ-ВО САМОСТ. КОРРЕКТИРОВОК', ttUInt32, 4, false, ffd11AndLess);
  AddTag(1149, 'количество корректировок по предписанию', 'КОЛ-ВО КОРРЕКТИРОВОК ПО ПРЕДПИС.', ttUInt32, 4, false, ffd11AndLess);
  AddTag(1151, 'сумма коррекций НДС по ставке 20%', 'СУММА КОРРЕКЦИЙ НДС 20%', ttVLN, 8, false, ffd11AndLess);
  AddTag(1152, 'сумма коррекций НДС по ставке 10%', 'СУММА КОРРЕКЦИЙ НДС 10%', ttVLN, 8, false, ffd11AndLess);
  AddTag(1153, 'сумма коррекций НДС по расч. ставке 20/120', 'СУММА КОРРЕКЦИЙ НДС ПО РАСЧ. СТ. 20/120', ttVLN, 8, false, ffd11AndLess);
  AddTag(1154, 'сумма коррекций НДС расч. ставке 10/110', 'СУММА КОРРЕКЦИЙ НДС ПО РАС. СТ. 10/110', ttVLN, 8, false, ffd11AndLess);
  AddTag(1155, 'сумма коррекций с НДС по ставке 0%', 'СУММА КОРРЕКЦИЙ НДС 0%', ttVLN, 8, false, ffd11AndLess);
  //
  ...
  // Старый тег
  AddTag(1162, 'код товарной номенклатуры', 'КОД ТОВАРН. НОМЕНКЛ.', ttByteArray, 32, false, ffd11AndLess);
  ...
  AddTag(1177, 'наименование основания для коррекции', 'НАИМЕН. ОСН. ДЛЯ КОРРЕКЦ', ttString, 256, false, ffd11AndLess);
  ...
  // Старый тег
  AddTag(1184, 'сумма коррекций без НДС', 'СУММА КОРРЕКЦ. БЕЗ НДС', ttVLN, 8, false, ffd11AndLess);
  //
  AddTag(1187, 'место расчетов', 'МЕСТО РАСЧЕТОВ', ttString, 256);
  // Старый тег
  AddTag(1188, 'версия ККТ', 'ВЕРСИЯ ККТ', ttString, 8, false, ffd11AndLess);
  //
  ...
  AddTag(1197, 'единица измерения предмета расчета', 'ЕД. ИЗМЕР. ПРЕДМЕТА РАСЧ.', ttString, 16, false, ffd11AndLess);
  ...
  // ФН 1.1
  AddTag(1227, 'получатель (покупатель)', 'ПОЛУЧАТЕЛЬ', ttString, 128);
  AddTag(1228, 'ИНН получателя (покупателя)', 'ИНН ПОЛУЧАТЕЛЯ', ttString, 12, True);
  AddTag(1229, 'акциз', 'АКЦИЗ', ttVLN, 6);
  AddTag(1230, 'код страны происхождения', 'КОД СТРАНЫ ПРОИСХ.', ttString, 3, True);
  AddTag(1231, 'номер таможенной декларации', 'НОМЕР ТАМОЖ. ДЕКЛ.', ttString, 32);
  ...
  AddTag(1243, 'дата рождения покупателя', 'ДАТА РОЖДЕНИЯ ПОКУПАТЕЛЯ', ttString, 10); // ДД.ММ.ГГГГ
  AddTag(1244, 'гражданство', 'ГРАЖДАНСТВО', ttString, 3); // ЦЦЦ
  AddTag(1245, 'код вида документа', 'КОД ВИДА ДОКУМЕНТА', ttString, 2); // ЦЦ
  AddTag(1246, 'данные документа', 'ДАННЫЕ ДОКУМЕНТА', ttString, 64); // {С}
  AddTag(1254, 'адрес покупателя', 'АДРЕС ПОКУПАТЕЛЯ', ttString, 256); // {С}
  AddTag(1256, 'сведения о покупателе', 'СВЕДЕНИЯ О ПОКУПАТЕЛЕ', ttSTLV, 1024); // {С}
  AddTag(1260, 'отраслевой реквизит предмета расчета', 'РЕКВИЗИТ ПРЕДМЕТА РАСЧЕТА', ttSTLV, 302);
  AddTag(1261, 'отраслевой реквизит чека', 'ОТРАСЛЕВОЙ РЕКВИЗИТ ЧЕКА', ttSTLV, 302);
  AddTag(1262, 'идентификатор ФОИВ', 'ИДЕНТИФИКАТОР ФОИВ', ttString, 3); // ССС
  AddTag(1263, 'дата документа основания', 'ДАТА ДОКУМЕНТА ОСНОВАНИЯ', ttString, 10); // ЦЦ.ЦЦ.ЦЦЦЦ в формате ДД.ММ.ГГГГ
  AddTag(1264, 'номер документа основания', 'НОМЕР ДОКУМЕНТА ОСНОВАНИЯ', ttString, 32); // {С}
  AddTag(1265, 'значение отраслевого реквизита', 'ЗНАЧЕНИЕ ОТРАСЛЕВОГО РЕКВИЗИТА', ttString, 256); // {С}
  AddTag(1270, 'операционный реквизит чека', 'ОПЕРАЦИОННЫЙ РЕКВИЗИТ ЧЕКА', ttSTLV, 144);
  AddTag(1271, 'идентификатор операции', 'ИДЕНТИФИКАТОР ОПЕРАЦИИ', ttByte, 1);
  AddTag(1272, 'данные операции', 'ДАННЫЕ ОПЕРАЦИИ', ttString, 64); // {С}
  AddTag(1273, 'дата операции', 'ДАТА ОПЕРАЦИИ', ttUnixTime, 4); // UnixTime
  AddTag(1274, 'дополнительный реквизит ОР', 'дополнительный реквизит ОР', ttString, 32);
  AddTag(1275, 'дополнительные данные ОР', 'дополнительные данные ОР', ttByteArray, 32);
  ...
  AddTag(1290, 'условия применения ККТ', 'условия применения ККТ', ttBitMask, 4);
  AddTag(1291, 'дробное количество', 'дробное количество', ttSTLV, 52);
  AddTag(1292, 'дробная часть', 'дробная часть', ttString, 24); // {Ц}/{Ц}
  ...
  AddTag(1301, 'КТ EAN-8', 'КТ EAN-8', ttString, 8); // {Ц}
  AddTag(1302, 'КТ EAN-13', 'КТ EAN-13', ttString, 13); // {Ц}
  AddTag(1303, 'КТ ITF-14', 'КТ ITF-14', ttString, 14); // {Ц}
  AddTag(1304, 'КТ GS1.0', 'КТ GS1.0', ttString, 38);
  AddTag(1305, 'КТ GS1.М', 'КТ GS1.М', ttString, 38);
  AddTag(1306, 'КТ КМК', 'КТ КМК', ttString, 38);
  AddTag(1307, 'КТ МИ', 'КТ МИ', ttString, 20);
  AddTag(1308, 'КТ ЕГАИС-2.0', 'КТ ЕГАИС-2.0', ttString, 33);
  AddTag(1309, 'КТ ЕГАИС-3.0', 'КТ ЕГАИС-3.0', ttString, 14);
  ...
  // Маркировка
  AddTag(2000, 'код маркировки', 'код маркировки', ttByteArray, 256);
  AddTag(2001, 'номер запроса', 'номер запроса', ttUInt32, 4);
  AddTag(2002, 'номер уведомления', 'номер уведомления', ttUInt32, 4);
  AddTag(2003, 'планируемый статус товара', 'планируемый статус товара', ttByte, 1);
  AddTag(2004, 'результат проверки КМ', 'результат проверки КМ', ttBitMask, 1);
  AddTag(2005, 'результаты обработки запроса', 'результаты обработки запроса', ttBitMask, 1);
  AddTag(2006, 'результаты обработки уведомления', 'результаты обработки уведомления', ttByte, 1);
  AddTag(2007, 'данные о маркированном товаре', 'данные о маркированном товаре', ttSTLV, 512);

  AddTag(2010, 'код маркировки товара', 'КМ', ttString, 200);
  AddTag(2011, 'контрольно-идентификационный знак товара', 'КИЗ', ttString, 134);
  AddTag(2012, 'идентификационный знак товара', 'ИД', ttString, 25);
  AddTag(2013, 'режим контроля выбытия товара', 'РК', ttByte, 1);
  AddTag(2014, 'новый статус кода маркировки', 'НОВЫЙ СТАТУС КМ', ttByte, 1);
  AddTag(2015, 'идентификатор кода проверки КИЗ', 'КПКИЗ ИД', ttUint16, 2);
  AddTag(2016, 'симметричный проверочный код', 'КПКИЗ С', ttByteArray, 4);
  AddTag(2017, 'асимметричный проверочный код', 'КПКИЗ А', ttByteArray, 64);
  AddTag(2018, 'результат проверки КМ', 'ПРОВЕРКА КМ', ttByte, 1); ///
  AddTag(2019, 'признак ошибки кода маркировки', 'ОШИБКА КМ', ttByte, 1);
  AddTag(2020, 'тип кода маркировки', 'СИМВОЛИКА', ttByte, 1);
  AddTag(2050, 'код идентификации товара SGTIN-198', 'SGTIN-198', ttString, 25, True);
  AddTag(2051, 'код идентификации товара SGTIN-96', 'SGTIN-96', ttString, 25, True);
  AddTag(2052, 'код идентификации вида товара', 'GTIN', ttString, 14, True);
  AddTag(2053, 'код идентификации вида товара, вложенного в упаковку', 'CONTENT', ttString, 14, True);
  AddTag(2054, 'дата производства товара', 'PROD DATE', ttString, 6, True);
  AddTag(2055, 'дата и время производства товара', 'PROD TIME', ttString, 12);
  AddTag(2056, 'срок годности товара', 'EXPIRY', ttString, 6, True);
  AddTag(2057, 'дата и время годности товара', 'EXPIRY DATE', ttString, 10);
  AddTag(2058, 'серийный номер товара', 'SERIAL', ttString, 20);
  AddTag(2059, 'номер серии/партии', 'BATCH/LOT', ttString, 20);
  AddTag(2060, 'дополнительные идентификационные данные товара', 'ADDITIONAL ID', ttString, 30);
  AddTag(2061, 'количество единиц товара', 'COUNT', ttString, 8);
  AddTag(2062, 'место производства товара (глобальный идентификатор)', 'PROD/SERV LOC', ttString, 13, True);
  AddTag(2099, 'реквизит ассоциации GS1', 'GS1 AI', ttString, 36, True);

  AddTag(2100, 'тип кода маркировки', 'тип кода маркировки', ttByte, 1, True);
  AddTag(2101, 'идентификатор товара', 'идентификатор товара', ttString, 38, True);
  AddTag(2102, 'режим обработки кода маркировки', 'режим обработки кода маркировки', ttByte, 1, True);
  AddTag(2104, 'количество непереданных уведомлений', 'количество непереданных уведомлений', ttUInt32, 4, True);
  AddTag(2105, 'коды обработки запроса', 'коды обработки запроса', ttByte, 1, True);
  AddTag(2106, 'результат проверки сведений о товаре', 'результат проверки', ttBitMask, 1, True);
  AddTag(2107, 'результаты проверки', 'результаты проверки', ttByte, 1, True);
  AddTag(2108, 'мера количества', 'мера количества', ttByte, 1, True);
  AddTag(2109, 'ответ ОИСМ о статусе товара', 'ответ ОИСМ о статусе товара', ttByte, 1, True);
  AddTag(2110, 'присвоенный статус товара', 'присвоенный статус товара', ttByte, 1, True);
  AddTag(2111, 'коды обработки уведомления', 'коды обработки уведомления', ttByte, 1, True);
  AddTag(2112, 'признак некорректных кодов', 'признак некорректных кодов', ttBitMask, 1, True);
  AddTag(2113, 'признак некорректных запросов', 'признак некорректных запросов', ttBitMask, 1, True);
  AddTag(2114, 'дата и время запроса', 'дата и время запроса', ttUnixTime, 4, True);
  AddTag(2115, 'контрольный код КМ', 'контрольный код КМ', ttString, 4, True);
  AddTag(2116, 'вид операции', 'вид операции', ttBitMask, 1, True);

  AddTag(2200, 'код страны для КИЗ на основе ТН ВЭД', 'КОД СТРАНЫ ДЛЯ КИЗ НА ОСНОВЕ ТН ВЭД', ttString, 2, True);
  AddTag(2201, 'код товара в системе идентификации на основе ТН ВЭД', 'КОД ТНВЭД', ttString, 100);
  AddTag(2202, 'номер серии партии в системе идентификации на основе ТН ВЭД', 'ПАРТИЯ/СЕРИЯ', ttString, 100);
  AddTag(2203, 'серийный номер продукта в системе идентификации на основе ТН ВЭД', 'СЕР НОМЕР', ttString, 100);
  AddTag(2299, 'дополнительный реквизит', 'РАСШ КИЗ', ttString, 100);
end;
