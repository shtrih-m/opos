...
2.	Для каждого товара (платежа) в чеке дополнительной строкой печатать название налоговой ставки (1199).
    Pyfxtybt

3.	Удалить столбец "НДС" с буквенным идентификатором налоговой группы. Столбец "Итог" сдвинуть вправо на освободившееся место. Расположение остальных столбцов не изменяется.
4.	В блоке сумм по налоговым группам удалить буквенные обозначения налоговых групп. Названия налоговых групп сдвинуть влево на освободившееся место.
5.	Печатать для субарендных платежей реквизиты покупателя (1227 и 1228). Выводить отдельными строками после названия платежа и названия налоговой ставки. Если название покупателя не умещается на одну строку, то переносить на следующую.
6.	Для субарендных платежей исключить печать полей "Плательщик", "ИНН", "По договору" и "В том числе НДС" после подвала чека. Т.к. информация повторяется в строке чека и в Квитанции.
7.	В квитанцию для субарендных платежей добавить в строку "Принято от:" ИНН плательщика через запятую после наименования плательщика.
