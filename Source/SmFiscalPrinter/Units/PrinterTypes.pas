...
const
  ...
  PrinterDiscountText   = 'ÑÊÈÄÊÀ';
  PrinterChargeText     = 'ÍÀÄÁÀÂÊÀ';
  PrinterItemVoidText   = 'ÑÒÎÐÍÎ';
...
type
  ...
  TFSSale = record
    ...
    Discount: Int64;
    Barcode: Int64;
    Text: string;         // Íàçâàíèå òîâàðà
    AdjText: string;      // Íàçâàíèå ñêèäêè èëè íàäáàâêè
  end;
  ...
  TReceiptFormat = record
    Valid: Boolean;
    Enabled: Boolean;
    TextItem: TReceiptFormatItem;            // ÍÀÈÌÅÍÎÂÀÍÈÅ Â ÎÏÅÐÀÖÈÈ
    QuantityItem: TReceiptFormatItem;        // ÊÎËÈ×ÅÑÒÂÎ X ÖÅÍÓ Â ÎÏÅÐÀÖÈÈ
    DepartmentItem: TReceiptFormatItem;      // ÑÅÊÖÈß Â ÎÏÅÐÀÖÈÈ
    AmountItem: TReceiptFormatItem;          // ÑÒÎÈÌÎÑÒÜ Â ÎÏÅÐÀÖÈÈ
    StornoItem: TReceiptFormatItem;          // ÍÀÄÏÈÑÜ ÑÒÎÐÍÎ Â ÎÏÅÐÀÖÈÈ
    DscText: TReceiptFormatItem;             // ÒÅÊÑÒ Â ÑÊÈÄÊÅ
    DscName: TReceiptFormatItem;             // ÍÀÄÏÈÑÜ ÑÊÈÄÊÀ
    DscAmount: TReceiptFormatItem;           // ÑÓÌÌÀ ÑÊÈÄÊÈ
    CrgText: TReceiptFormatItem;             // ÒÅÊÑÒ Â ÍÀÄÁÀÂÊÅ
    CrgName: TReceiptFormatItem;             // ÍÀÄÏÈÑÜ ÍÀÄÁÀÂÊÀ
    CrgAmount: TReceiptFormatItem;           // ÑÓÌÌÀ ÍÀÄÁÀÂÊÈ
    DscStornoText: TReceiptFormatItem;       // ÒÅÊÑÒ Â ÑÒÎÐÍÎ ÑÊÈÄÊÈ
    DscStornoName: TReceiptFormatItem;       // ÍÀÄÏÈÑÜ ÑÒÎÐÍÎ ÑÊÈÄÊÈ
    DscStornoAmount: TReceiptFormatItem;     // ÑÓÌÌÀ ÑÒÎÐÍÎ ÑÊÈÄÊÈ
    CrgStornoText: TReceiptFormatItem;       // ÒÅÊÑÒ Â ÑÒÎÐÍÎ ÍÀÄÁÀÂÊÈ
    CrgStornoName: TReceiptFormatItem;       // ÍÀÄÏÈÑÜ ÑÒÎÐÍÎ ÍÀÄÁÀÂÊÈ
    CrgStornoAmount: TReceiptFormatItem;     // ÑÓÌÌÀ ÑÒÎÐÍÎ ÍÀÄÁÀÂÊÈ
  end;
  ...
  TFSCalcReport = record
    DocNumber: Integer;             // Íîìåð ÔÄ: 4 áàéòà
    FiscalSign: Integer;            // Ôèñêàëüíûé ïðèçíàê: 4 áàéòà
    OutstandDocCount: Integer;      // Êîëè÷åñòâî íåïîäòâåðæä¸ííûõ äîêóìåíòîâ: 4 áàéòà
    OutstandDocDate: TPrinterDate;  // Äàòà ïåðâîãî íåïîäòâåðæä¸ííîãî äîêóìåíòà: 3 áàéòà ÃÃ,ÌÌ,ÄÄ
  end;

  { TFSWriteStatus }

  TFSWriteStatus = record
    IsConnected: Boolean;       // Áèò 0 – òðàíñïîðòíîå ñîåäèíåíèå óñòàíîâëåíî
    HasMessageToSend: Boolean;  // Áèò 1 – åñòü ñîîáùåíèå äëÿ ïåðåäà÷è â ÎÔÄ
    IsWaitForTicket: Boolean;   // Áèò 2 – îæèäàíèå îòâåòíîãî ñîîáùåíèÿ (êâèòàíöèè) îò ÎÔÄ
    IsServerCommand: Boolean;   // Áèò 3 – åñòü êîìàíäà îò ÎÔÄ
    ConnParamsChanged: Boolean; // Áèò 4 – èçìåíèëèñü íàñòðîéêè ñîåäèíåíèÿ ñ ÎÔÄ
    IsWaitForAnswer: Boolean;   // Áèò 5 – îæèäàíèå îòâåòà íà êîìàíäó îò ÎÔÄ
  end;
  ...
  TPayments = array [0..3] of Int64;
  TTaxTotals = array [0..4] of Int64;
  TVatRates = array [0..4] of Int64;
  ...
  TFSCorrectionReceipt = record
    RecType: Byte;
    Total: Int64;
    ResultCode: Integer;      // Êîä îøèáêè: 1 áàéò
    ReceiptNumber: Integer;   // Íîìåð ÷åêà: 2 áàéòà
    DocumentNumber: Integer;  // Íîìåð ÔÄ: 4 áàéòà
    DocumentMac: Integer;     // Ôèñêàëüíûé ïðèçíàê: 4 áàéò
  end;
  ...
  TFSTicket = record
    Number: Integer;
    Data: string;
    Date: TPrinterDateTime;   // Äàòà è âðåìÿ             DATE_TIME      5
    DocumentMac: string;      // Ôèñêàëüíûé ïðèçíàê ÎÔÄ   DATA           18
    DocumentNum: Integer;     // Íîìåð ÔÄ                 Uint32, LE     4
  end;
...
function FSGetErrorText(Code: Integer): string;
begin
  case Code of
...
function FMGetErrorText(Code: Integer): string;
begin
  case Code of
// 0x00
    $00: Result := 'No errors';
...
begin
  if Code in [0..$20] then
    Result := FMGetErrorText(Code)
  else
    Result := GetErrorText2(Code);
end;

function GetErrorTextFS(Code: Integer): string;
begin
  if (Code in [0..$1F])or(Code in [$30..$32]) then
    Result := FSGetErrorText(Code)
  else
    Result := GetErrorText2(Code);
end;
