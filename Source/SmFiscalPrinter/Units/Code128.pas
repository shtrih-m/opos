...
type
  TBarcodeAlignment = (baCenter, baLeft, baRight);
  TBarcodeType = (bcCode128A, bcCode128B, bcCode128C);
  ...
  TBarcodeRec = record
    ...
    BarcodeType: TBarcodeType;    // Barcode type
    Alignment: TBarcodeAlignment; // Barcode alignment
  end;
...
function GeTBarcode128Name(BT: TBarcodeType): string;
begin
  case BT of
    bcCode128A: Result := 'Code128A';
    bcCode128B: Result := 'Code128B';
    bcCode128C: Result := 'Code128C';
  else
    Result := 'Unknown barcode type';
  end;
end;

procedure RaiseInvalidCharError(BT: TBarcodeType; const S: string);
begin
  raise Exception.CreateFmt('''%s'' - incorrect character for barcode %s',
    [S, GeTBarcode128Name(BT)]);
end;
...
function TBarcode128.GetBarcode128Types: string;
var
  i: TBarcodeType;
begin
  Result := '';
  for i := bcCode128A to bcCode128C do
  begin
    if Result <> '' then Result := Result + #13#10;
    Result := Result + GeTBarcode128Name(i);
  end;
end;

function TBarcode128.FindCode128A(c: char): Integer;
var
  v: Char;
  i: Integer;
begin
  ...
  RaiseInvalidCharError(bcCode128A, C);
end;

function TBarcode128.FindCode128B(c: char): Integer;
var
  v: Char;
  i: Integer;
begin
  ...
  RaiseInvalidCharError(bcCode128B, C);
end;

// find Code 128 Codeset C

function TBarcode128.FindCode128C(c: string): Integer;
var
  i: Integer;
begin
  Result := 0;
  ...
  RaiseInvalidCharError(bcCode128C, C);
end;

// Create Code128A code

procedure TBarcode128.CreateCode128A;
var
  i: Integer;
  Index: Integer;
  startcode: string;
...
procedure TBarcode128.CreateCode128B;
var
  i: Integer;
  Index: Integer;
  startcode: string;
...
procedure TBarcode128.CreateCode128C;
var
  S: string;
  i: Integer;
  j: Integer;
...
procedure TBarcode128.AlignLeft;
begin
...
procedure TBarcode128.AlignRight;
begin
...
procedure TBarcode128.AlignCenter;
var
  DotCount: Integer;
...
procedure TBarcode128.MakeAlignment;
begin
...
procedure TBarcode128.MakeBarWidth;
var
  S: string;
  i: Integer;
begin
...
procedure TBarcode128.CreateBarcode;
begin
  // Barcode creation
  case Barcode.BarcodeType of
    bcCode128A: CreateCode128A;
    bcCode128B: CreateCode128B;
    bcCode128C: CreateCode128C;
  else
