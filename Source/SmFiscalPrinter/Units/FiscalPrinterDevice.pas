...
(*
ѕроверка  маркированного товара FF61H
 од команды FF61h. ƒлина сообщени€: 9 байт.
ѕароль оператора: 4 байта

Ќовый статус товара: 1 байт
1 - "—формирован".Ќе выдан регистратору.
2 - "√отов". ¬ыдан регистратору, но не применен.
3 - "¬ыдан".  ћ выдан “— дл€ нанесени€. ѕрименение не подтверждено.
4 - "¬ыпущен".  ћ нанесен на товар или упаковку, правильность нанесени€ кода подтверждена, маркированный товар произведен.
5 - "Ќе использован".  ћ не был выдан “— к моменту закрыти€ заказа.
6 - "”пакован". “овар или упаковка с данным  ћ находитс€ в составе логистической единицы.
7 - "–аспакован". ћаркированный объект находитс€ в обороте или в употреблении в виде товарной единицы.
8 - ¬ыбыл по определенным, известным участникам обращени€ товара, причинам на этапе производства (например, отобран, как опытный образец дл€ испытаний), оптового или розничного оборота (уничтожен безвозвратно в составе логистической единицы, похищенной, испорченной в совокупности со всем содержимым и т.п.).
9 - "¬ыбыл через розничную сеть".
10 - "¬ состо€нии выбыти€" (мерный товар).
11 - "”тер€н".
12 - "ќборот приостановлен".
13 - "ќборот запрещен".
14 - "ѕотреблен".
15 - "ƒублирован".
ƒлина кода маркировки: 1 байт
–ежим проверки: 1 байт
0 - полна€ проверка.
1 - только онлайн проверка.
2 - только локальна€ проверка.
¬ первую очередь всегда надо пытатьс€ проводить полную проверку. ѕолна€ проверка состоит из 2-х этапов, локальна€ проверка и онлайн проверка. ≈сли локальна€ проверка дала отрицательный результат, то   “ прекращает проверку и сообщает об этом управл€ющему ѕќ. ƒалее в зависимости от режима контрол€ и пожеланий покупател€ можно дл€ данного  ћ произвести онлайн проверку.
≈сли локальна€ проверка выполнена успешно то   “ (в режиме передачи данных) автоматически произведет онлайн проверку.
ƒл€   “ в автономном режиме онлайн проверка не производитс€.
”правл€ющее ѕќ исход€ из результатов проверки  ћ, режима выбыти€ дл€ данного товара и пожеланий покупател€ должно принимать решение о регистрации или отказе в регистрации данного предмета расчета.
–ежим "только локальна€ проверка" нужен на переходный период, пока не определены правила по онлайн проверке.

(данные должны быть загружены командой 0xDD)
ќтвет: FF61h	    ƒлина сообщени€: 8 байт.
 од ошибки: 1 байт
–езультат локальной проверки кода маркировки: 1 байт
0 - проверка не проводилась, (дл€ симметричной криптографической системы).
1 - код маркировки проверен, достоверный.
2 - код маркировки проверен, недостоверный.
3 - проверка не проводилась, (криптографическа€ система асимметрична€, но в ‘Ќ-ћ нет ключа с идентификатором  ѕ »«.ид)
 од обработки пакета: 1 байт.
пол€ ниже имеют смысл, если в этом поле 0, в противном случае должны игнорироватьс€
–азрешение на продажу товара от »—ћ: 1 байт.
0 - товар разрешен к продаже
1 - товар запрещен к продаже
—татус  ћ: 1 байт (см. выше)
 од ошибки от сервера  ћ: 1 байт
0 - —татус успешно изменен
1 -  »« отсутствует в базе —ерверы — « ћ или  »« отсутствует в базе »—ћ
2 - Ќе корректен формат  »«
3 -  риптографическа€ проверка  ѕ »« дала отрицательный результат
4 -  »« имеет в базе —ерверы — « ћ статус не совместимый с запрашиваемым изменением. Ќапример, запрошено изменение статуса "¬ыбыл в розничной сети" в то врем€, как товар уже был продан. »ными словами, запрашиваетс€ запрещенное изменение статуса кода маркировки
5 - ¬ списке вложени€ обнаружены ошибки
—татус проверок сервера : 1 байт
0 - все хорошо.
Ћюбое другое значение - все плохо.
“ип символики: 1 байт
0 - ассиметрична€
1 - симметрична€
2 - табачна€
...
ѕрив€зка  маркированного товара к позиции FF67H

 од команды FF67h. ƒлина сообщени€: 5+N байт.
  ѕароль оператора: 4 байта
  ƒлина кода маркировки: 1 байт
  ƒанные маркировки N байт
  ƒанна€ команда должна вызыватьс€ после прив€зки всех тегов к предмету расчета.

ќтвет: FF67h	    ƒлина сообщени€: 4 байт.
   од ошибки: 1 байт
  первые 2 байта значени€ реквизита "код товара": 2 байта,
  “ип Data Matrix:1 байт.
  0 -  ћ 88,
  1- ћ симметричный,
  2- ћ “абачный,
  3- ћ 44.
