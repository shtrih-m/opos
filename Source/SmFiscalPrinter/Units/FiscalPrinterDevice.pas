    ...
    try
      ...
      begin
        ...
        if (i = (Parameters.MaxRetryCount-1)) then
          raise ECommunicationError.Create(_('Ќет св€зи'));
      end;
    end;
...
(******************************************************************************
Ќачать закрытие смены FF42H

 од команды FF42h. ƒлина сообщени€: 6 байт.
  ѕароль системного администратора: 4 байта
ќтвет: FF42h ƒлина сообщени€: 1 байт.
   од ошибки: 1 байт
...
(******************************************************************************
  ...
  Ј	Device name (X bytes) AnsiString of WIN1251 code page characters;
...
(*******************************************************************************
  ...
  Ј	Result Code (1 byte)
  Ј	ECR model (16 bytes) AnsiString of WIN1251 code page characters
...
(*
ќпераци€ V2 FF46H
 од команды FF46h . ƒлина сообщени€:  160 байта.
ѕароль: 4 байта
“ип операции: 1 байт
1 - ѕриход,
2 - ¬озврат прихода,
3 - –асход,
4 - ¬озврат расхода
 оличество: 6 байт ( 6 знаков после зап€той )
÷ена:             5 байт
—умма операции 5 байт *
Ќалог:           5 байт **
Ќалогова€ ставка:  1 байт
Ќомер отдела: 1 байт
0Е16 - режим свободной продажи, 255 - режим продажи по коду товара
ѕризнак способа расчЄта : 1 байт
ѕризнак предмета расчЄта: 1 байт
Ќаименование товара: 0-128 байт ASCII
...
(*
«апрос параметра открыти€ ‘Ќ
 од команды 	FF0Eh. ƒлина сообщени€: 9 байт.
ѕароль системного администратора (4 байта)
ѕор€дковый номер отчета о регистрации/перерегистрации (1 байт)
Ќомер тега (“ип “, TLV параметра) (2 байта)
если T=FFFFh2, то читать TLV структуру командой FF3Bh
ќтвет: 		FF0Eh. ƒлина сообщени€: 3+X1 байт.
 од ошибки (1 байт)
TLV структура (X1 байт)
ѕримечание:
1 - длина ответного сообщени€ зависит от TLV структуры, возвращаемой ‘Ќ на заданный номер тега (кроме FFFFh);
2 - при запросе всех тегов TLV структура не возвращаетс€ (X=0).
...
(*
ќтменить документ в ‘Ќ FF08H
 од команды FF08h. ƒлина сообщени€: 6 байт.
ѕароль системного администратора: 4 байта
ќтвет: FF08h ƒлина сообщени€: 1 байт.
 од ошибки: 1 байт
*)
...
(*
0xF4    «апрос необнул€емых сумм.
¬торой параметр отвечает за тип запрашиваемых сумм
"	FE F4 00 00 00 00 - возвращает 4 8-ми байтных числа (приход, возврат прихода, расход, возврат расхода). Ёто Ќ— без деталировки по типам оплаты
"	FE F4 01 00 00 00 - возвращает 16 8-ми байтных числа (приход). Ёто Ќ— с деталировкой по 16-ти типам оплаты
"	FE F4 02 00 00 00 - возвращает 16 8-ми байтных числа (возврат прихода). Ёто Ќ— с деталировкой по 16-ти типам оплаты
"	FE F4 03 00 00 00 - возвращает 16 8-ми байтных числа (расход). Ёто Ќ— с деталировкой по 16-ти типам оплаты
"	FE F4 04 00 00 00 - возвращает 16 8-ми байтных числа (возврат расхода). Ёто Ќ— с деталировкой по 16-ти типам оплаты
"	FE F4 05 00 00 00 - возвращает 4 8-ми байтных числа (коррекци€ прихода, коррекци€ возврата прихода, коррекци€ расхода, коррекци€ возврата расхода)
ќтвет.  ƒлина сообщени€: 33(129) байт.
"	 од ошибки (1 байт)
"	ƒанные (X байт). X в зависимости от типа запрашиваемых данных.
...
(*
—формировать чек коррекции V2 FF4AH
 од команды FF4Ah . ƒлина сообщени€: 69 байт.
ѕароль системного администратора: 4 байта
“ип коррекции :1 байт
ѕризнак расчета:1байт
—умма расчЄта :5 байт
—умма по чеку наличными:5 байт
—умма по чеку электронными:5 байт
—умма по чеку предоплатой:5 байт
—умма по чеку постоплатой:5 байт
—умма по чеку встречным представлением:5 байт
—умма Ќƒ— 20%:5 байт
—умма Ќƒ— 10%:5 байт
—умма расчЄта по ставке 0%:5 байт
—умма расчЄта по чеку без Ќƒ—:5 байт
—умма расчЄта по расч. ставке 20/120:5 байт
—умма расчЄта по расч. ставке 10/110:5 байт
...
(*
«акрытие чека расширенное вариант V2 FF45H
 од команды FF45H. ƒлина сообщени€: 182 байт.
ѕароль системного администратора: 4 байта
—умма наличных (5 байт)
—умма типа оплаты 2 (5 байт)
—умма типа оплаты 3 (5 байт)
—умма типа оплаты 4 (5 байт)
—умма типа оплаты 5 (5 байт)
—умма типа оплаты 6 (5 байт)
—умма типа оплаты 7 (5 байт)
—умма типа оплаты 8 (5 байт)
—умма типа оплаты 9 (5 байт)
—умма типа оплаты 10 (5 байт)
—умма типа оплаты 11 (5 байт)
—умма типа оплаты 12 (5 байт)
—умма типа оплаты 13 (5 байт)
—умма типа оплаты 14 (5 байт) (предоплата)
—умма типа оплаты 15 (5 байт) (постоплата)
—умма типа оплаты 16 (5 байт) (встречное представление)
ќкругление до рубл€ в копейках (1 байт)
Ќалог 1 (5 байт) (Ќƒ— 20%)
Ќалог 2 (5 байт) (Ќƒ— 10%)
ќборот по налогу 3 (5 байт) (Ќƒ— 0%)
ќборот по налогу 4 (5 байт) (Ѕез Ќƒ—)
Ќалог 5 (5 байт) (Ќƒ— расч. 18/118)
Ќалог 6 (5 байт) (Ќƒ— расч. 10/110)
—истема налогообложени€(1 байт)
“екст (0-64 байт)
_______________________________________________________
ѕримечани€:
“ипы оплаты 2-13 при передаче в ќ‘ƒ суммируютс€ и передаютс€ как оплата "ЁЋ≈ “–ќЌЌџћ»".
¬ режиме начислени€ налогов 0 ( 1 “аблица) касса рассчитывает налоги самосто€тельно исход€ из проведенных в документе операций и налоги переданные в команде игнорируютс€. ¬ режиме начислени€ налогов 1 налоги должны быть об€зательно переданы из верхнего ѕќ.

ќтвет:   FF45h ƒлина сообщени€: 14 байт.
 од ошибки: 1 байт
—дача ( 5 байт)
Ќомер ‘ƒ :4 байта
‘искальный признак: 4 байта
...
-------------------------------------------------------------------------------
«акрытие чека расширенное вариант є2

 од команды  FF45h. ƒлина сообщени€: 118 - 182 байта или 202 байта 4
- ѕароль системного администратора (4 байта)
- —умма наличных (5 байт)
- —умма типа оплаты 2 (5 байт)
- —умма типа оплаты 3 (5 байт)
...
- —умма типа оплаты 12 (5 байт)
- —умма типа оплаты 13 (5 байт)
- —умма типа оплаты 14 (5 байт) аванс 5
- —умма типа оплаты 15 (5 байт) кредит
- —умма типа оплаты 16 (5 байт) встречное представление
- ќкругление до рубл€ в копейках (1 байт)
- Ќалог 1 (5 байт) Ќƒ— 18%
- Ќалог 2 (5 байт) Ќƒ— 10%
- ќборот по налогу 3 (5 байт) Ќƒ— 0%
- ќборот по налогу 4 (5 байт) Ѕез Ќƒ—
- Ќалог 5 (5 байт) Ќƒ— расч. 18/118
- Ќалог 6 (5 байт) Ќƒ— расч. 10/110
- —истема налогообложени€ (1 байт)1
- Ѕит 0 Ц ќ—Ќ
- Ѕит 1 Ц ”—Ќ доход
- Ѕит 2 Ц ”—Ќ доход минус расход
- Ѕит 3 Ц ≈Ќ¬ƒ
- Ѕит 4 Ц ≈—ѕ
- Ѕит 5 Ц ѕ—Ќ
- “екст (0-64 байт) 4
- Ќалог 7 (5 байт) Ќƒ— 5% 4
- Ќалог 8 (5 байт) Ќƒ— 7% 4
- Ќалог 9 (5 байт) Ќƒ— расч. 5/105 4
- Ќалог 10 (5 байт) Ќƒ— расч. 7/107 4

ќтвет:  FF45h.  ƒлина сообщени€: 16 (21) байт2.
-  од ошибки (1 байт)
- —дача (5 байт)
- Ќомер ‘ƒ (4 байта)
- ‘искальный признак (4 байта)
- ƒата и врем€ (5 байт) DATE_TIME3
...
(******************************************************************************
  —формировать отчЄт о регистрации   “

   од команды FF06h . ƒлина сообщени€: 40 байт.
  ѕароль системного администратора: 4 байта
  »ЌЌ : 12 байт ASCII
  –егистрационный номер   “: 20 байт ASCII
   од налогообложени€: 1 байт
  –ежим работы: 1 байт

  ќтвет: FF06h ƒлина сообщени€: 9 байт.
   од ошибки: 1 байт
  Ќомер ‘ƒ: 4 байта
  ‘искальный признак: 4 байта
...
(*
«апросить фискальный документ в TLV формате
 од команды FF3јh . ƒлина сообщени€: 10 байт.
  ѕароль системного администратора: 4 байта
  Ќомер фискального документа: 4 байта
ќтвет: FF3јh ƒлина сообщени€: 5 байт.
   од ошибки: 1 байт
  “ип фискального документа: 2 байта STLV
...
(*
„тение TLV фискального документа
 од команды FF3Bh . ƒлина сообщени€: 6 байт.
  ѕароль системного администратора: 4 байта
ќтвет: FF3Bh ƒлина сообщени€: 1+N байт.
   од ошибки:1 байт
...
begin
  ...
  try
    ...
    if not SaleEnabled then
      raiseError(E_SALE_NOT_ENABLED, _('ѕродажа товара запрещена'));
  finally
    Client.Free;
  end;
end;
...
(*
Ќачать формирование чека коррекции
 од команды FF35h . ƒлина сообщени€: 6 байт.
ѕароль системного администратора: 4 байта
ќтвет: FF35h ƒлина сообщени€: 1 байт.
 од ошибки: 1 байт
*)
...
(******************************************************************************
  ѕроверка маркированного товара

   од команды 	FF61h. ƒлина сообщени€: 10+X+Y байт.
  ѕароль оператора (4 байта)
  ¬ходные данные (4+X+Y байт)

  —остав входных данных:

  —мещение	ƒлина	ѕараметр	«начение
  0	1 байт	ѕланируемый статус	“ег 2003
  1	1 байт	–ежим обработки	“ег 2102 (сейчас всегда "0")
  2	1 байт	ƒлина кода маркировки ( ћ) в байтах (X)	ѕолна€ длина  ћ
  3	1 байт	ƒлина списка TLV в байтах	ѕолна€ длина списка TLV
  4	X байт	 ћ	—ам  ћ, как он был прочитан сканером
  4+X	Y байт	—писок TLV	≈сли планируетс€ частичное выбытие маркированного
            товара (согласно с тегом 2003), то необходимо сформировать буфер
            из тегов 2108 (мера) и 1023 (количество) и передать его здесь

  ќтвет:		FF61h. ƒлина сообщени€: 9+X байт.
   од ошибки (1 байт)
  –езультат проверки (6+N байт)

  —остав данных результата проверки:

  —мещение	ƒлина	ѕараметр	«начение	ѕримечание
  0	1 байт	—татус локальной проверки	“ег 2004
  1	1 байт	ѕричина, по которой не была проведена локальна€ проверка
  2	1 байт	–аспознанный тип  ћ	“ег 2100
  3	1 байт	ƒлина дополнительных параметров	ƒлина данных, идущих далее
            ≈сли автономный режим или если сервер не ответил в течение таймаута, то "0"
  4	1 байт	 од ответа ‘Ќ на команду онлайн-проверки
            ¬ соответствии и вводом ошибки ‘Ќ	≈сли 0x20, то в следующем байте
            возвращаетс€ причина в соответствии с ѕримечанием 2 ниже
  5	1 байт	–езультат проверки  ћ	“ег 2106	“олько если сервер ответил без ошибок
  6	N байт	—писок реквизитов ответа сервера	TLV List	“олько если сервер ответил без ошибок
...
(******************************************************************************
  ...
  —инхронизировать регистры со счЄтчиком ‘Ќ

   од команды 	FF62h. ƒлина сообщени€: 6 байт.
    ѕароль системного администратора (4 байта)

  ќтвет: 		FF62h. ƒлина сообщени€: 3 байта.
       од ошибки (1 байт)
...
(******************************************************************************
  ...
  «апрос ресурса свободной пам€ти в ‘Ќ

   од команды 	FF63h. ƒлина сообщени€: 6 байт.
  ѕароль системного администратора (4 байта)

  ќтвет: 		FF63h. ƒлина сообщени€: 11 байт.
   од ошибки (1 байт)
  –есурс данных 5 летнего хранени€ 1 (4 байта)
  –есурс данных 30 дневного хранени€ 2 (4 байта)
  –есурс дл€ хранени€ уведомлений о реализации маркированного товара3 (1 байт)
  ѕримечание:
  1 - ќриентировочное количество документов, которые можно создать в ‘Ќ.
  2 - –азмер свободной области (в килобайтах) дл€ записи документов 30 дней хранени€.
      ѕосле 30 дней работы значение может колебатьс€ на посто€нном уровне.
  3 - ѕроцент заполнени€ области хранени€ уведомлений о реализации маркированных
      товаров дл€ ќ»—ћ. ѕараметр не возвращаетс€ если ‘Ќ зарегистрирован в
...
(******************************************************************************
  ...
  ѕередача в ‘Ќ TLV из буфера

   од команды 	FF64h. ƒлина сообщени€: 6 байт.
  ѕароль системного администратора (4 байта)

  ќтвет: 		FF64h. ƒлина сообщени€: 3 байта.
   од ошибки (1 байт)
  ѕримечание:
  ѕозвол€ет передать в ‘Ќ предварительно загруженную в буфер TLV структуру.
  ѕозвол€ет передать TLV длиннее 250 байт. Ѕуфер тот же что и дл€ проверки маркировки.
...
(******************************************************************************
  ...
  ѕолучить случайную последовательность

   од команды 	FF65h. ƒлина сообщени€: 6 байт.
  ѕароль оператора (4 байта)

  ќтвет: 		FF65h. ƒлина сообщени€: 19 байт.
   од ошибки (1 байт)
  ƒанные (16 байт)
...
(******************************************************************************
  јвторизоватьс€
   од команды 	FF66h. ƒлина сообщени€: 22 байта.
  ѕароль оператора (4 байта)
  ƒанные дл€ авторизации (16 байт)

  ќтвет: 		FF66h. ƒлина сообщени€: 3 байта.
   од ошибки (1 байт)
...
(******************************************************************************
  ...
  ѕрив€зка маркированного товара к позиции

   од команды 	FF67h. ƒлина сообщени€: 7+N байт.
  ѕароль оператора (4 байта)
  ƒлина кода маркировки (1 байт)
  ƒанные маркировки (N байт)
  ѕризнак ќ—” (1 байт) 1 ќбъемно-сортовой учет (ќ—”)

  ќтвет: 		FF67h. ƒлина сообщени€: 6+(6+N)4 байт.
   од ошибки (1 байт)
  –аспознанный тип кода (2 байта) 2
  “ип Data Matrix (1 байт) 3
  0 -  ћ 88
  1 -  ћ симметричный
  2 -  ћ “абачный
  3 -  ћ 44
  0 xFF- GS-1 без маркировки
  –езультат проверки (6+N байт)4 может отсутствовать (см. ѕримечание 1)

  ѕримечание:
  1 - ћожет отсутствовать. ƒл€ реализации позиции по ќ—” надо передать значение 0xFF.
  2 - “аблица допустимых значений:
  “ип кода	«начение
  EAN8	0x45 0x08
  EAN13	0x45 0x0D
  ITF14	0x49 0x0E
  ...
  ќ—” EAN8		0x4F 0x08
  ќ—” EAN13	0x4F 0x0D
  ќ—” GTIN ITF14	0x4F 0x0E
  Ќераспознанный код	0x00 0x00
  3 - ѕоле имеет смысл только если код распознан как GS-1 Data Matrix, в противном случае принимает значение 0.
  4 - ¬ случае если код маркировки не провер€лс€ ранее командой FF61h "ѕроверка маркированного товара",   “ сама еЄ подаст и добывит ответ на неЄ к ответу на команду FF67h.
  ...
  —мещение	ƒлина	ѕараметр	«начение	ѕримечание
  0	1 байт	—татус локальной проверки	“ег 2004
  1	1 байт	ѕричина, по которой не была проведена локальна€ проверка	—м. ѕримечание 2 ниже
  2	1 байт	–аспознанный тип  ћ	“ег 2100
  3	1 байт	ƒлина дополнительных параметров	ƒлина данных, идущих далее	≈сли автономный режим, то "0"
  4	1 байт	 од ответа ‘Ќ на команду онлайн-проверки	¬ соответствии и вводом ошибки ‘Ќ	≈сли 0x20, то в следующем байте возвращаетс€ причина в соответствии с ѕримечанием 3 ниже
  «начение 0xFF, если сервер не ответил в течение таймаута.
  5	1 байт	–езультат проверки  ћ	“ег 2106	“олько если сервер ответил без ошибок
  6	N байт	—писок реквизитов ответа сервера	TLV List	“олько если сервер ответил без ошибок
...
(******************************************************************************
  ...
  ѕолучить состо€ние по передаче уведомлений о реализации маркированных товаров
   од команды 	FF68h. ƒлина сообщени€: 6 байт.
  ѕароль оператора (4 байта)
  ќтвет:		FF68h. ƒлина сообщени€: 13 байт.
   од ошибки (1 байт)

  Ќаименование	“ип	ƒлина	ќписание
  —осто€ние по передачи уведомлений	Byte	1	0 - нет активного обмена;
  1 - начато чтение уведомлени€;
  2 - ожидание квитанции на уведомление;
   оличество уведомлений в очереди	Uint16, LE	2	0, если на все уведомлени€ была получена квитанци€
  Ќомер текущего уведомлени€	Uint32, LE	4	Ќомер уведомлени€ дл€ передачи, или уведомлени€, на которое ожидаетс€ квитанци€
  ƒата и врем€ текущего уведомлени€	DATE_TIME	5	0, если на все уведомлени€ получена квитанци€
  ѕроцент заполнени€ области хранени€ уведомлений	Byte	1

   од ошибки (1 байт) может принимать следующие значени€:
   од ответа	ќписание	 омментарий (действи€   “)

  00h	Ќет ошибки

  02h	Ќеверное состо€ние ‘Ќ	‘Ќ был активирован в автономном режиме или уже
      начато чтение уведомлени€ и состо€ние по передаче уведомлений имеет
      значение "1" - "начато чтение уведомлени€": необходимо завершить или
      отменить начатое чтение уведомлени€

  32h	«апрещена работа с маркированными товарами	ѕри регистрации ‘Ќ не был
...
(******************************************************************************
  ...
  ѕрин€ть или отвергнуть введенный код маркировки FF69H

   од команды FF69h. ƒлина сообщени€: 7 байт.
  ѕароль оператора: 4 байта
  –ешение : 1 байт. 0 - отвергнуть, 1 - прин€ть, 2 - очистить буфер  ћ
   оманду необходимо подавать после проверки каждого  ћ.

  ќтвет: FF69h	    ƒлина сообщени€: 1 байт.
   од ошибки: 1 байт
...
(******************************************************************************
  ...
  «апрос статуса по работе с кодами маркировки

   од команды 	FF70h. ƒлина сообщени€: 6 байт.
  ѕароль оператора (4 байта)

  ќтвет:		FF70h. ƒлина сообщени€: 11 байт.
   од ошибки (1 байт)
  —татус работы с кодами маркировки (8 байт)

  Ќаименование	“ип	ƒлина	ќписание
  —осто€ние по проверке  ћ	Byte	1	0 - “аблица проверки  ћ переполнена
    1 - Ќет  ћ на проверке
    2 - ѕередан  ћ в команде B1h
    3 - —формирован запрос о коде маркировки при помощи команды B5h
    4 - ѕолучен и передан в ‘Ќ ответ на запрос при помощи команды B6h
  —осто€ние по формированию уведомлени€ о реализации маркированного товара	Byte	1
    0 - ”ведомление о реализации маркированного товара не формируетс€
    1 - Ќачато формирование уведомлени€ о реализации маркированного товара
    2 - ‘ормирование уведомлений заблокировано из-за переполнени€ области временного хранени€
  ‘лаги разрешени€ команд работы с  ћ	Byte	1	—м. таблицу "‘лаги разрешени€ команд работы с  ћ" ниже
   оличество сохранЄнных результатов проверки  ћ	Byte	1
   оличество  ћ, результаты проверки которых сохранены в ‘Ќ командой B2h c кодом "1"
   оличество  ћ, включенных в уведомление о реализации маркированного товара	Byte	1
  ѕредупреждение о заполнении области хранени€ уведомлений о реализации маркированного товара	Byte	1
    ¬ этом параметре ‘Ќ информирует   “ о заполнении области хранени€ уведомлений о реализации маркированного товара.
    ¬озможные следующие значени€ параметра:
    0 - ќбласть заполнена менее чем на 50%
    1 - ќбласть заполнена от 50 до 80%
    2 - ќбласть заполнена от 80 до 90%
    3 - ќбласть заполнена более чем на 90%
    4 - ќбласть полностью заполнена, формирование новых уведомлений невозможно
   оличество уведомлений в очереди	Uint16, LE	2
     оличество неподтверждЄнных или невыгруженных уведомлений о реализации маркированного товара
  ‘лаги разрешени€ команд работы с  ћ

  Ѕит 7	Ѕит 6	Ѕит 5	Ѕит 4	Ѕит 3	Ѕит 2	Ѕит 1	Ѕит 0	 од разрешенной команды
  0	0	0	0	0	0	0	1	B1h
  0	0	0	0	0	0	1	0	B2h
  0	0	0	0	0	1	0	0	B3h
  0	0	0	0	1	0	0	0	B5h
...
(******************************************************************************
  ...
  Ќачать выгрузку уведомлений о реализации маркированных товаров (в автономном режиме)

   од команды 	FF71h. ƒлина сообщени€: 6 байт.
  ѕароль оператора (4 байта)

  ќтвет:		FF71h. ƒлина сообщени€: 11 байт.
   од ошибки (1 байт)
  ќбщее число уведомлений (2 байта)
  Ќомер первого уведомлени€ (4 байта)
  –азмер первого уведомлени€ (2 байта)
...
(******************************************************************************
  ...
  ѕрочитать блок уведомлени€ (в автономном режиме)

   од команды 	FF72h. ƒлина сообщени€: 6 байт.
  ѕароль оператора: 4 байта

  ќтвет:		FF72h. ƒлина сообщени€: 11+N байт.
   од ошибки (1 байт)
  Ќомер текущего уведомлени€ (4 байта)
  ѕолный размер текущего уведомлени€ (2 байта)
  —мещение от начала текущего уведомлени€ (2 байта)
  Ѕлок данных (N байт)

  ѕримечание:
    “ выполн€ет поблочное чтение всех доступных уведомлений
  (максимально   “ может прочитать блок 128 байт).
  —ледует вызывать команду до получени€ ошибки "Ќет данных" или на основании
  общего числа уведомлений, полученного подачей команды FF71h.
  ƒопускаетс€ прочитать лишь часть уведомлений и подтвердить их.
  ¬ любой момент до подтверждени€ чтени€ можно вызвать команду FF71h и
...
(******************************************************************************
  ...
  ѕодтвердить выгрузку уведомлени€ (в автономном режиме)

   од команды 	FF73h. ƒлина сообщени€: 14 байт.
  ѕароль оператора (4 байта)
  Ќомер уведомлени€ (4 байта) получаетс€ из ответа на команду FF72h
  CRC16 (4 байта) контрольна€ сумма уведомлени€

  ќтвет:		FF73h. ƒлина сообщени€: 3 байта.
   од ошибки (1 байт)
...
(******************************************************************************
  ...
   од команды 	FF74h. ƒлина сообщени€: 6 байт.
  ѕароль оператора (4 байта)

  ќтвет:		FF74h. ƒлина сообщени€: 51 байт.
   од ошибки (1 байт)
  —трока исполнени€ ‘Ќ  (48 байт)  ASCII
...
(******************************************************************************
  ...
  «апрос общего размера данных документа в ‘Ќ

   од команды 	FF75h. ƒлина сообщени€: 14 байт.
  ѕароль оператора (4 байта)
  ќтвет:		FF75h. ƒлина сообщени€: 11 байт.
   од ошибки (1 байт)
  –азмер в байтах текущего документа дл€ ќ‘ƒ ( 4 байта)
  –азмер в байтах текущего уведомлени€ о реализации маркированных товаров дл€ ќ»—ћ ( 4 байта)
...
procedure TFiscalPrinterDevice.CheckPrinterStatus;
  ...
  begin
    Result := Tnt_WideFormat('%s: %d, %s', [_('Ќевозможно изменить состо€ние'), Mode, GetModeText(Mode)]);
  end;
