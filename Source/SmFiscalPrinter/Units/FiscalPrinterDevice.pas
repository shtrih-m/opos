...
—умма расчЄта по расч. ставке 18/118:5 байт
...
(*
ƒанные в массиве представлены в виде строки, в которой:
первые 2 байта - код справочника;
последующие 6 байт - код идентификации группы товара;
последние 24 байта - код идентификации экземпл€ра товара
*)
...
begin
  ...
  case MarkType of
    ...
    3:
    begin // Ћекарственные препараты
      Serial := Copy(GS1Barcode.Serial, 1, 24);
      Data := ReverseString(IntToBin(3, 2)) + GTIN + Serial;
      Data := TTLVTag.Int2ValueTLV(1162, 2) + TTLVTag.Int2ValueTLV(Length(Data), 2) + Data;
    end;
    5:
    begin // “абачные издели€
      Serial := Copy(GS1Barcode.Serial, 1, 24);
...
(******************************************************************************
  ...
   од команды FF67h. ƒлина сообщени€: 7 байт.
  ѕароль оператора: 4 байта
  ƒлина кода маркировки: 1 байт
  (данные должны быть загружены командой 0xDD)
    “ провер€ет был ли данный  ћ проверен ранее. ≈сли был то  ћ включаетс€ в
  состав предмета, независимо от результата проверки. ≈сли  ћ ранее не провер€лс€,
  то будет возвращена ошибка и  ћ не будет прив€зан к предмету расчета.
  ƒанна€ команда должна вызыватьс€ после прив€зки всех тегов к предмету расчета.

  ќтвет: FF67h	    ƒлина сообщени€: 8 байт.
   од ошибки: 1 байт
  –езультат локальной проверки кода маркировки: 1 байт
   од обработки пакета : 1 байт.
  –азрешение на продажу товара от »—ћ : 1 байт.
