...
// name            opcode    opnd? meaning
 ...
 OPEN        = TREOp (37); // -    Mark this point in input as start of \n
                           //      OPEN + 1 is \1, etc.
  ...
  while scan <> nil do begin
     ...
     case scan^ of
         ...
         ANYLETTER: begin
            if (reginput^ = #0) or
             (Pos (scan^, fWordChars) <= 0) //###0.940
             { not ((reginput^ >= 'a') and (reginput^ <= 'z')  !! I've forgotten (>='0') and (<='9')
         ...
         NOTLETTER: begin
            if (reginput^ = #0) or
             (Pos (scan^, fWordChars) > 0) //###0.940
              { (reginput^ >= 'a') and (reginput^ <= 'z')  !! I've forgotten (>='0') and (<='9')
                 or (reginput^ >= 'A') and (reginput^ <= 'Z')
                 or (reginput^ = '_')}
             then EXIT;
            inc (reginput);
...
function TRegExpr.GetLinePairedSeparator : RegExprString;
 begin
  if fLinePairedSeparatorAssigned
   then Result := fLinePairedSeparatorHead + fLinePairedSeparatorTail
   else Result := '';
